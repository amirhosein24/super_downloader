    ...
    try:
        ...
        if not acquired:
            context.bot.send_message(
                Admin, f"{chat_id} - failed to acquire lock")
        ...
        context.bot.send_message(
            Admin, f"error in try var lock, error: {error}")
        update.message.reply_text('error happend, pls try again later.')
        return
...
def link_handler(update, context):
    try:
        ...
        if link.startswith("https://www.youtube.com") or link.startswith("https://youtube.com") or link.startswith("https://youtu.be"):
            ...
            files, caption = twitter_downer.main_download(
                context, chat_id, link)

        elif link.startswith("https://open.spotify.com/"):
            waitmessage = update.message.reply_text(
                "processing ...", reply_to_message_id=update.message.message_id)
            details, trackid = spotify_downer.get_track_details(link)
            if details:
                context.bot.edit_message_text(
                    chat_id=chat_id, message_id=waitmessage.message_id, text=details, reply_markup=keyboards.spotify_key(trackid))
                callback = True
...
def download_callback(query, context):
    ...
    try:
        if platform == "spotify":
            context.bot.edit_message_text(
                chat_id=chat_id, message_id=query.message.message_id, text="download started ...")
        ...
        if files:  # send the files after downloading them
            ...
            context.bot.edit_message_text(
                chat_id=chat_id, message_id=query.message.message_id, text="nothin found to download")
